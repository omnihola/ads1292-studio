...
| 实时绘图 | QCustomPlot（经 `IWaveformPlot` 抽象） | 功能最接近 pyqtgraph；QPainter 光栅渲染（**非** GPU/VBO）；内部使用不触发 GPL |
...
### 绘图后端抽象（GPL 保险）
定义 `IWaveformPlot` 接口（`appendSamples / setTimeWindow / setYRange / setAutoscale / overlayEvents / setPaperSpeed / clear`）。GUI 只依赖接口；QCustomPlot 是当前唯一实现。**风险提示**：若将来设备连同软件交付外部即构成 GPL 分发，届时换 QtCharts/自绘后端只改此层。
...
| **P2** | CSV(live/raw) + HDF5 读写兼容（不丢数据优先） | C++ 写文件可被 Python 读、Python 文件可被 C++ 读；schema/datasets/attrs/hashes 对拍；往返一致 |
| **P3** | 采集 worker：先 simulator（确定性样本流），后真实 QSerialPort | simulator 流→样本入队→CSV journal 落盘；真实板连通后 live/raw 两模式采集 |
| **P4** | Live GUI：status 面板 + event console + waveform（原始波形 + SNR 条） | 连 simulator/真板→实时波形可见→录制 CSV→事件标注 |
| **P5** | DSP：filters / R peaks / HR / SNR / spectrum / PQRST（含 P4 的实时显示滤波回填） | 全部数值对拍 Python；HP/Notch/LP/QRS 显示滤波接入 Live GUI |
...
**P4/P5 依赖说明**：Live GUI 显示滤波（HP/Notch/LP/QRS）是因果实时 IIR，依赖 DSP。P4 只保证原始波形 + SNR 可见；所有滤波统一在 P5 落地并对拍后回填 P4 界面。
