...
@dataclass(frozen=True)
class RecordingManifestPlanRow:
    relative_path: str
    manifest_status: str
    csv_path: Path
    target_path: Path
...
class SessionIndexExport:
    ...
    manifest_plan_csv_path: Path
    manifest_plan_html_path: Path
    manifest_apply_script_path: Path
...
def export_session_index(
    ...
    manifest_plan_csv_path = output / f"{stamp}-{slug}-manifest-plan.csv"
    manifest_plan_html_path = output / f"{stamp}-{slug}-manifest-plan.html"
    manifest_apply_script_path = output / f"{stamp}-{slug}-apply-manifests.sh"
    ...
    return SessionIndexExport(
        ...
        manifest_plan_csv_path=manifest_plan_csv_path,
        manifest_plan_html_path=manifest_plan_html_path,
        manifest_apply_script_path=manifest_apply_script_path,
        rows=rows,
        summary=summary,
        sidecar_plan_rows=sidecar_plan_rows,
        sidecar_template_paths=sidecar_template_paths,
        manifest_plan_rows=manifest_plan_rows,
    )
...
def write_recording_manifest_apply_script(path: Path | str, rows: tuple[RecordingManifestPlanRow, ...]) -> Path:
    script_path = Path(path)
    lines = [
        "#!/bin/sh",
        "set -eu",
        "",
        "# Generated by ADS1292 Studio.",
        "# Rebuild recording manifests after reviewing CSV and sidecars.",
        "",
    ]
    if not rows:
        lines.append("echo 'No recording manifests to rebuild.'")
    for row in rows:
        csv_path = row.csv_path.resolve()
        lines.extend(
            [
                f"# {row.relative_path}: {row.manifest_status}",
                f"python -m ads1292_studio manifest {quote(str(csv_path))}",
                "",
            ]
        )
    script_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def build_recording_manifest_repair_plan(
    rows: tuple[SessionIndexRow, ...],
) -> tuple[RecordingManifestPlanRow, ...]:
    return tuple(
        RecordingManifestPlanRow(
            relative_path=row.relative_path,
            manifest_status=row.recording_manifest_status,
            csv_path=row.path,
            target_path=row.path.with_suffix(".manifest.json"),
            suggested_action=_recording_manifest_action(row.recording_manifest_status),
        )
        for row in rows
        if row.recording_manifest_status != "pass"
    )
...
def summarize_rows(rows: tuple[SessionIndexRow, ...]) -> SessionIndexSummary:
    return SessionIndexSummary(
        ...
        total_annotated_samples=sum(row.total_annotated_samples for row in rows),
...
def _row_for_csv(path: Path, root: Path) -> SessionIndexRow | None:
    ...
    event_summary = _event_summary_for(path, recording.sample_rate_hz)
    ...
    return SessionIndexRow(
        ...
        total_annotated_samples=event_summary.total_annotated_samples,
...
def _event_summary_for(csv_path: Path, sample_rate_hz: float = DEFAULT_EVENT_SAMPLE_RATE_HZ) -> EventAnnotationSummary:
    ...
    if events_json.exists():
        return _summarize_events(read_events_json(events_json), sample_rate_hz=sample_rate_hz)
    if events_csv.exists():
        return _summarize_events(read_events_csv(events_csv), sample_rate_hz=sample_rate_hz)
    ...
    *,
    ...
    for event in events:
        ...
        if marker.duration_seconds > 0:
            ...
            total_annotated_samples += event_sample_indices(marker, sample_rate_hz)["duration_samples"]
...
def _html(title: str, rows: tuple[SessionIndexRow, ...], summary: SessionIndexSummary) -> str:
    ...
    for row in rows:
        values = [
            ...
            str(row.total_annotated_samples),
            row.event_labels,
...
<body>
  ...
  <p>Annotated recordings: {summary.annotated_recordings} | Event annotations: {summary.event_annotations} | Interval annotations: {summary.interval_event_annotations} | Annotated seconds: {summary.total_annotated_seconds:.2f} | Annotated samples: {summary.total_annotated_samples}</p>
...
def _recording_manifest_plan_html(title: str, rows: tuple[RecordingManifestPlanRow, ...]) -> str:
    body = []
    for row in rows:
        values = [row.relative_path, row.manifest_status, str(row.target_path), row.suggested_action]
        body.append("<tr>" + "".join(f"<td>{escape(value)}</td>" for value in values) + "</tr>")
    return f"""<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  <title>{escape(title)} Recording Manifest Repair Plan</title>
  <style>
    body {{ font-family: -apple-system, BlinkMacSystemFont, sans-serif; margin: 32px; }}
    table {{ border-collapse: collapse; width: 100%; }}
    th, td {{ border: 1px solid #ddd; padding: 6px 8px; text-align: left; }}
...
<body>
  <h1>{escape(title)} Recording Manifest Repair Plan</h1>
  <p>Recording manifest repair tasks: {len(rows)}</p>
  <table>
    <tr><th>File</th><th>Manifest Status</th><th>Target Path</th><th>Suggested Action</th></tr>
    {"".join(body)}
  </table>
</body>
</html>
...
def _recording_manifest_action(status: str) -> str:
    if status == "missing":
        return "Write recording manifest"
    if status == "fail":
        return "Refresh stale recording manifest"
    return "Review and rewrite recording manifest"
