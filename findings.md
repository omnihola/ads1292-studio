...
| Generalize the background-thread-plus-result-queue pattern to Connect and Start/Stop | The same `CsvLoadResult`/queue pattern already proven for CSV loading fixes the Connect-blocks-the-GUI-thread bug and the Start-declares-success-before-confirmation bug with one consistent mechanism instead of three different ad hoc patches. |
| Remove the cross-thread `device.close()` call from `LiveWorker.stop()` instead of adding more synchronization around it | The worker thread already closes its own device via its `with Ads1x9xDevice(...) as device:` block when it notices `stop_event`; the extra caller-thread `close()` call was redundant and racy, not load-bearing. |
| Make offline review build full-recording numpy arrays directly from loaded samples instead of routing through the live-display deques | The live deques are intentionally bounded (`maxlen=5000`) for real-time rolling display; reusing them for one-shot offline review silently capped every loaded recording's analysis to its final ~10 seconds. Decimating only the matplotlib line (not the metrics) keeps rendering fast without losing analysis accuracy. |
