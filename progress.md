...
| GUI status cards TDD red check | `conda run -n sensor python -m pytest tests/test_gui_control_state.py -q` before implementation | Missing structured status card helper | `ImportError: cannot import name 'gui_status_cards'` | Pass |
| GUI status cards focused test | `conda run -n sensor python -m pytest tests/test_gui_control_state.py -q` | Control states, hints, overview text, and structured status-card tones match disconnected, streaming, and unsaved-data workflows | 16 passed | Pass |
| GUI status tone style red check | `conda run -n sensor python -m pytest tests/test_gui_control_state.py -q` before implementation | Missing deterministic tone-to-style helper | `ImportError: cannot import name 'status_tone_style'` | Pass |
| GUI status cards focused retest | `conda run -n sensor python -m pytest tests/test_gui_control_state.py -q` | Status tone style mapping and status cards pass together | 17 passed | Pass |
| GUI status cards related tests | `conda run -n sensor python -m pytest tests/test_gui_quality.py tests/test_gui_quality_gate_config.py tests/test_gui_session_index.py tests/test_gui_scroll.py tests/test_gui_layout.py tests/test_gui_control_state.py -q` | Existing GUI helper/layout tests pass after rendering Status Overview as styled rows | 28 passed | Pass |
...
| Real recording signal-quality focused red check | `conda run -n sensor python -m pytest tests/test_csv_io.py tests/test_gui_control_state.py -q` before implementation | Missing signal-quality card helper and canonical lead-off merge | `ImportError: cannot import name 'gui_signal_quality_cards'` | Pass |
| Real recording signal-quality focused test | `conda run -n sensor python -m pytest tests/test_csv_io.py tests/test_gui_control_state.py -q` | CSV import preserves explicit `lead_off_bits`; GUI cards expose actual recording summary | 25 passed | Pass |
| Real recording signal-quality smoke | `conda run -n sensor env PYTHONPATH=src python -c "..."` against `recordings/2026-06-18-221342-ads1292-studio.csv` | Actual recording maps to `Good ECG/QRS on CH2`, `99.9% OK, 28 bad`, `86.7 bpm, 129 R` | Passed; Matplotlib cache warning only | Pass |
| Real recording signal-quality final verification | `conda run -n sensor python -m pytest tests/test_csv_io.py tests/test_gui_control_state.py tests/test_gui_quality.py -q`; `conda run -n sensor python -m pytest -q`; `py_compile`; `git diff --check` | Signal-quality cards, canonical lead-off import, existing GUI quality text, syntax, and whitespace all verify after metrics reuse refactor | 28 passed; 92 passed | Pass |
| ADS1292R synchronized three-panel GUI red check | `conda run -n sensor python -m pytest tests/test_gui_layout.py -q` before implementation | Missing fixed plot-layout helper | `ImportError: cannot import name 'ads1292r_plot_layout_labels'` | Pass |
...
| 2026-06-18 | Actual exported recording was usable but the GUI did not expose that clearly | 1 | Added Status-tab signal-quality cards for ECG source/label, contact, HR/R peaks, and artifact metrics. |
...
- **Iter 3 (data detail, validates iter 2):** added a `RecordingInfoPanel` ("Info" tab) showing a detailed structured per-recording report from the full `QualityMetrics` (sample_count, duration, sample rate, ECG source, quality label, contact% + bad samples, R peaks, HR median/min/max, QRS/P/T flags, baseline drift, noise RMS, peak-to-peak, CH1/CH2 channel scores). Plugged in via the renderer registry (one tab line + one registry entry — no load-path edits), validating the iter-2 extensibility. Verified on the real 44,884-sample recording. 538 tests pass.
...
- Actions taken:
  - Added `ui_qt/analysis_panels.py`: `PqrstPanel` (R-aligned average beat via `pqrst_review`/`review_channels`), `SpectrumPanel` (FFT power + amplitude histogram via `build_spectrum_analysis`), `EventLogPanel` (runtime log). Wired into the PQRST Beat / Spectrum / Event Log tabs.
  - `_show_recording` now also renders PQRST + spectrum and feeds the runtime log; Event Log tab receives drained log lines each tick.
  - Backported the reliable-finalization fix to the Tk app: `app.py _close()` joins the writer thread and finalizes json+xlsx before destroy (root cause: `_close` previously stopped the worker and destroyed immediately, so closing during/after a recording left it CSV-only).
  - Verified end-to-end on the real 44,884-sample recording: offline Review + Signal-quality cards (Good ECG/QRS, CH2, 99.9% contact, 86.7 bpm, 129 R — matches the documented baseline), PQRST (beats=129), and spectrum all render. Screenshots: `docs/mockups/2026-06-21-pyqt-{spectrum,pqrst}.png`.
- Verification: full suite **535 passed**.
- Next: real-hardware smoke (`ads1292-studio-qt`: record -> close window without Stop -> confirm csv+json+xlsx in `~/Documents/ECG/live`); Phase 44.3 (wire left-form archive actions: Export Report/Package/Verify/Batch/Session Index; Validation/Protocol forms; event sidecar persistence + plot overlay); Phase 44.4 polish.

## 5-Question Reboot Check
| Question | Answer |
|----------|--------|
| Where am I? | Phase 44.2 complete: Qt app has offline Load CSV -> Review + quality cards + PQRST + Spectrum + Event Log, live SNR, and reliable CSV+JSON+XLSX finalization (on Stop and on window close) to `~/Documents/ECG/{live,raw}`; the same finalize fix is backported to the Tk app. Verified end-to-end on the real 44,884-sample recording (metrics match baseline). 535 tests pass. Next: real-hardware smoke + Phase 44.3 (left-form archive actions, Validation/Protocol forms, event persistence/overlay). Phase 44.1 and earlier complete. Channel-label confirmation (which exact Einthoven lead CH2 represents) is still pending the user's electrode-disconnect hardware test. |
